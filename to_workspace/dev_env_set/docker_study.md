...
> stop 상태는 잠시 PC 전원을 끈 상태와 비슷하다. 컨테이너에 저장된 파일과 변경사항들은 저장된채로 남는다.

`docker container rm {컨테이너명(NAME)}` : 해당 컨테이너 삭제. (삭제를 하기 전에 먼저 stop을 먼저 해야함.)

`docker container ls -a` : 이미 종료된(stop 상태) 컨테이너 포함해서 목록보기.는 아니고 종료된 것들 뿐만 아니라 다양한 상태의 컨테이너 전부 보기.
...
- 이 `-p` 옵션은 Port의 약자로, Port Mapping을 설정하는데 필요함. (컨테이너들은 전부 고유의 네트워크와 IP 주소를 보유한다) 이 포트를 기반으로 도커는 가상 네트워크에서 돌아가는데, 이 IP 포트 주소가 사전에 없으면 외부에서 접근하기가 어려움.
...
> 참고로 COPY로 폴더를 넣어서 빌드하고 싶은데 넣지말아할 것이 잇다면 `.dockerignore` 파일을 작성해서 제외시킬 수 있다.
...
기본적으로 위 basic 강의와 중복되는 점들이 있어 영상들을 쭉 보면서 새로운 사실과 팁을 위주로 작성.

### 다른 방식으로 컨테이너 실행하기.

Run이나 Compose말고 다른 방식도 존재한다.

`docker pull nginx`

- 도커 컨테이너를 만들지만 아직 실행하는 것은 아니다.

`docker create nginx`

- 이제 해당 컨테이너를 진짜 실행.

`docker start {컨테이너명}` <- nginx 기입하는거 아님

Run이나 Compose는 이미지를 보고 컨테이너를 바로 만드는데 create/start 하나의 텀을 준 것이다. stop 됐을 때의 단계, 즉 방에 월세 내고 자립는 잡혔지만, 안에서 뭐가 돌아가지는 않는 상태에서 한번 멈추고 시작하는 것이다.

> 만약 creat/start와 compose를 동시에 사용하고 싶으면 `docker-compose up --no-start`하고 `docker-compose start` 순으로 하면 된다.

- 실습해보기
```bash
docker pull bigenlight/practice-image:v1

docker create --name my-manual-container -p 5001:5000 bigenlight/practice-image:v1

docker container ls

docker start my-manual-container
```

### 외부에서 컨테이너로 접속하기

만약 컨테이너 내부 터미널로 들어가고 싶으면 아래 명령어를 사용하면 된다.

`docker exec -it {컨테이너명} bash`

```bash
PS C:\Users\tping\Documents\RWL_Intern> docker exec -it c7db bash
root@c7db6ded0a83:/app# ls
app.py
```

- `exit` : 컨테이너에서 나가기.

> 당연하지만 여러 터미널에서 접속해도 괜찮다.

> 한쪽에서는 터미널 접속을 하고, 동시에 외부 터미널에서 `docker logs -f1`를 통해 동시에 출력되는 로그를 지켜볼 수도 있다.

### VScode와 연계

당연하지만 vscode로 도커 내부로 들어갈 수 있으면 굉장히 좋을 것이다.

- 일단 devcontainer 익스텐션을 설치한다.

![alt text](images/docker_1_image.png)

- 측면 아래 >< 버튼이나 ctrl + shit + p를 눌러 Remote window 명령 팔레트를 킨다.

- Attach to Running Container...를 검색한다.

초반에는 아무 폴더도 보이지 않아 당황할 수도 있다.

- 터미널을 키고 현재 위치를 확인한다.

지금은 그냥 `root@c7db6ded0a83:`이다

우리가 `exic -it {이름} bash`으로 들어갔을 때는 아래와 같았다.

```
root@c7db6ded0a83:/app# 
```

- 해당 위치로, 만약 검색이 안되면 수동으로 `/app`를 쳐서 검색해서 폴더에 들어간다.

그러면 쨘~! 아래 처럼 해당 컨테이너, 그것도 우리가 넣었던 파일을 볼 수 있다.

![alt text](images/docker_1_image-1.png)
### 계획
- [x] 도커(Docker) 기초: 1~12강 시청
    - [x] 도커와 vscode
- [x] [인프런] 비전공자도 이해할 수 있는 Docker 입문/실전: 1.1~2.8 시청.
...
`docker container stop {컨테이너명(NAME)}` : 해당 컨테이너 중지. (잠시 중단하는 것, start으로 다시 킬 수 있음.)
...
> 참고로 퍼시스턴스 데이터는 지속적으로 필요한, 영속적인 데이터를 일컫는다.
...
> 참고로 `-o`는 아웃풋의 약자. 이 작동의 아웃풋을 tar 파일에 저장하라는 것임. 만약에 빼면 터미널에 바이너리가 콸콸나올거임 (누가 이렇게 쓰냐 싶지만 이 순수 바이너리를 그대로 통신으로 내보내고, 다른 PC에서 load에서 -i 빼고 그 즉시 인풋으로 받아들이 수도 있음).

> 참고로 tar 파일은 압축이 아닌, 여러 파일을 하나로 묶는 메타데이터 저장 방식임.
...
> 여기서 app.py를 편집할 수 있다. 이 컨테이너는 기본적으로 해당 파일을 ADD하고, run할 때 cmd로 add.py를 실행하는 간단한 구조이다. 그래서 해당 app.py를 수정하고. 현재 이미지를 commit하고, 그 이미지를 다시 실행시켜서 지속적으로 유지하거나 잠시 stop으롤 쉴 수도 있다. 이전 처럼 외부에서 수정하고, 다시 빌드하고 그럴 필요가 없다.

### 만약에 도커 컨테이너를 돌리던 중에 갑자기 PC 전원이 꺼지면?

이미 저장된 데이터들은 안전하게 저장된거니 안심해라.

## 도커 팁

- 참고로 docker stop이나 rm을 할때 컨테이너명이 아니라 아이디로도 앞 숫자로도 지정할 수 있다.

예를 들어 ID가 35e72.. 이면 35e로도 조작이 가능하다. (보통 3,4글자면 충분하다.)

![alt text](images/docker_image-30.png)

- 물론 위 팁은 image에도 해당된다.

`docker image rm 35e`도 된다.

- `docker image rm $(docker images -q)`: 현재 컨테이너로 작동 중인 이미지말고 이미지 전부 삭제. (전부다 삭제하고 싶으면 -f 추가)

- `docker kill {컨테이너명}`: stop이 안될때 강제종료 시키기.

- `docker logs {컨테이너명}`: 백그라운드(`-d`)에서 돌아가는 컨테이너의 로그 출력을 보는 명령어. (`--tail {숫자}`로 뒤에서 부터 몇 줄의 로그만 볼지도 결정 가능.)

- `docker logs -f {컨테이너명}`: 실시간으로 출력 되는 로그 보기.

### 도커 상태 터미널에서 색깔 보는 방법
