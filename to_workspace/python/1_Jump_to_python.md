...
0. 계획
1. [[점프 투 파이썬](https://wikidocs.net/5)]
2. [[파이썬 프로그래밍의 기초, 자료형](https://wikidocs.net/11)]
    - 숫자형
    - 8진수와 16진수
    - 문자열 자료형
    ...
    - 자료형의 값을 저장하는 공간, 변수
        - 변수명 규칙
3. [[프로그램의 구조를 쌓는다!](https://wikidocs.net/19)]
    - if문
    - while문
        - while-else
    - for문
...
4. [[파이썬의 입출력](https://wikidocs.net/23)]
    - 함수
    ...
    - 파일 읽고 쓰기
    - 프로그램의 입출력 - argv
5. [[파이썬 날개 달기](https://wikidocs.net/27)]
    - 클래스
        - 상속
        - 클래스 속의 변수들
    - 모듈
    ...
    - 외부 라이브러리
        - pip
6. [[파이썬 날아오르기](https://wikidocs.net/184208)]
    - 클로저와 데코레이터
        - 클로저
        - 데코레이터 (**중요!**)
    - 이터레이터와 제너레이터
...
- 여러개를 나오게하려면 하나의 이터러블로(리스트, 튜플, 딕셔너리) 나오게하면 됨.
...
- 객체의 진실
> 클래스로 만들어낸 오브젝트가 **객체**? 객체라는 용어를 다른 곳에서도 많이 사용하지 않았나? 맞다. 다른 곳에서도 쓰인다. 아니, 사실 파이썬의 거의 모든 것이 객체다! 심지어 int 숫자 `10` 이런 것도 객체다. 왜냐하면 이것도 다음과 같이 엄연히 매서드도 사용할 수 있다: `10.bit_length()`. "10" 또한 '10'이라는 이름을 가진 int 클래스로 만들어낸 엄연한 객체이다. 그걸 잘 못 느낄 뿐.
...
어차피 보통은 그뒤에도 실행되니 딱히 필요없지 않을까 싶기도한데.. 

예를 except를 특정 오류가 잡게 만들었는데, 해당 오류를 못 잡았을 때도 뒤에 finally는 무조건 실행됨. 이때는 필요함.
...
> 용어 정리: 리스트, 딕셔너리 등은 **이터러블**. 아터러블 안에 반복 가능한 다양한 객체의 종류가 있다. 그 중에서 특수하게 반복 가능한 **이터레이터**라는 반복 종류도 있는 것이다.
...
`-> int`는 함수가 어떤 타입을 return으로 반환하는지 명시하는 것. 근데 강제성은 없음. 그래도 에러가 나면 이를 보고 알려줌.
