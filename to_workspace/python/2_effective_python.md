...
- 물론 가장 쓸모 있을 때는 아마 무언가를 한번에 언패킹할 때일 것이다.
...
> 물론 `super().__init__()`의 역할이 이것만 있는데 아니다. `__init__`을 상속 받으면서 다시 정의하면 이것도 **매서드 오버라이딩**이라 **부모 클래스의 init이 덮어씌워지게** 되는데, 그러면 부모 클래스에서만 정의된 변수들을 못 받아올 수 있으니, **부모 클래스에 정의된 초기화 코드를 지금 여기서 실행**하라는 표시로도 사용된다.
