...
<img src="images/2026-03-18-22-12-44.png" width="600">

- Encoder는 인풋의 특징을 뽑아 저차원으로 압축함. 보통 1차원의 벡터임.

<img src="images/2026-03-19-11-30-21.png" width="600">

이를 **Latent space**라고 주로 말함.

- Decoder는 인풋에 대해서 생긴 벡터들에 대해서 복원을 시도함.

근데 이게 쉬운게 아님.

기존 특정 이미지에 대한 latent vector를 아래 그래프 $z_{ref}$라고 칭한다면.

<img src="images/2026-03-19-11-27-38.png" width="600">
...
> 분류문제와 CEE를 보다가 갑자기 이걸 보면 **헷갈림**. 분류문제는 애초에 아웃풋이 확률도 딱딱 나와서 쉬웠는데 **이건 그게 아님**. **x**가 특정 **이미지**일 때, $p(x)$는 % 단위의 확률이 **아니라**, **이 픽셀 조합이 우리가 가진 실제 데이터셋(예: 손글씨) 정답 데이터들 안에 얼마나 빽빽하게 속해 있는가?** 를 나타내는 **밀도 값**임(Density). x가 진짜 손글씨 같은 이미지라면 p(x) 값이 높게 나올 것. 그냥 아무 노이즈이면 p(x) 값이 아주 낮게 나오는 것임. 

- $p(z)$ : Prior Distribution (사전 확률 분포)
    - 데이터 $x$를 관측하기 전(Prior)에, 잠재 공간(Latent space)의 변수 $z$가 **따를 것이라고 미리 가정**해 두는 분포임.

<img src="images/2026-03-19-12-39-19.png" width="600">

- $p(z|x)$ : 특정 데이터 $x$가 주어졌을 때(조건), 그 $x$를 만들어냈을 법한 잠재 변수 $z$의 분포. (나중에 바뀌지만 아무튼 이게 인코더 역할)

- $p(x|z)$ : 잠재 변수 $z$가 주어졌을 때(조건), 그 $z$로부터 특정 데이터 $x$가 만들어질 확률입니다. (나중에 이게 디코더 역할)

> $p(x|z)$ 또한 확률을 띡 하나 내뱉는게 아님. 여기서는 이미지 전체 픽셀에 하나하나 개별적으로 어떤 값을 가져야하는지 나타내는 확률을 일괄적으로 만드는거임 (또는 분포라고 봐도 됨). 

- 하지만 문제는 우리는 이 $p(z)$가 어떻게 생겨먹었는지 모른다는 것.
    - 그래서 일단 가우시안 분포, 그것도 **표준** **정규분포** $\mathcal{N}(0, I)$의 형태일거라고 가정하는거임.

<img src="images/2026-03-19-12-50-59.png" width="600">

- 그리고 진짜 사후 확률 $p(z|x)$도 계산하기가 너무 어려워서, 이를 근사하는 함수 $q(z|x)$으로 바꿈. <- VAE 혁신 중 하나

$p(z|x) \approx q(z|x)$ <- 일단 어려우니까 대충 q라는 공식이라고 생각하고.

$q(z|x) = \mathcal{N}(\mu, \sigma)$ <- 그 q 공식 또한 가우시안 분포임, 하지만 **표준 정규분포는 아니며** $\mu, \sigma$는 **학습**해서 수정해야함. (물론 이 공식 그대로는 안씀)
...
<img src="images/2026-03-19-13-03-35.png" width="600">

> 이 이미지의 **Regularization 부분이 오해**를 불러일으킬 수 있는데, 저기 안의 '**|**'는 **조건부 확률이 아니라** KL divergence의 **두 분포**를 구별하는 용도임. 더 구별하기 쉽게 위에 내가 적은 공식 처럼 '$\Vert$' 으로 구별하는 방식으로 **널림 쓰임**.
...
<img src="images/2026-03-19-13-07-27.png" width="600">
...
- [x] 이 부분을 정확히 알기 위해서는 KL divergence 스터디 필요

- 일단 $q(z|x)$을 통해 무언가를 만들기는 했음.

<img src="images/2026-03-19-13-13-06.png" width="350">

- 하지만 우리는 p(z)가 결국에 가우시안 표준 정규분포를 따른다고 가정했기에, 이 $q(z|x)$ 최적화에 대한 종착지가 표준 정규분포가 됨.

<img src="images/2026-03-19-13-14-48.png" width="350">

- 근데 이게 걍 차이가 나는거임. KL-divergence 애초에 예측 분포(Q)와 실제 분포(P)가 얼마나 차이 나는지 보여주는 지표임. 

$D_{KL}(P, Q)$, 여기서는 $D_{KL}(q(z|x), p(z))$ 또는 $\text{KL}(q(z|x) \Vert p(z))$으로 표기하면 됨.

이 $D_{KL}(P, Q)$는 어떻게 구하냐고? 이론대로라면 아래 처럼 구하면 됨.

$$\sum_{s} p_s \log \left( \frac{p_s}{q_s} \right) = D_{\text{KL}}(P, Q)$$

- 근데 나중에 보면 알겠지만 Q와 P 둘다 가우시안 분포로 가정해버리고, 서로 평균과 표준편차만 달라서 계산이 매우 간단해짐.

$$\mathcal{L}_{KL} = -\frac{1}{2} \sum_{i=1}^{k} \left( 1 + \log(\sigma_i^2) - \mu_i^2 - \sigma_i^2 \right)$$

- $\mu_i$: 인코더가 예측한 평균
- $\sigma_i^2$: 인코더가 예측한 분산
- $k$: 잠재 공간 벡터의 차원 수
...
<img src="images/2026-03-19-13-17-17.png" width="500">
...
<img src="images/2026-03-19-14-04-15.png" width="350">

- 그 뿐만 아니라 여러 데이터 사이의 interpolation도 가능해져서, 이 특징의 이만큼, 다른 특징의 이만큼, 이런식으로도 담아내는 것이 가능해짐.

<img src="images/2026-03-19-14-05-54.png" width="400">
...
<img src="images/2026-03-19-14-07-01.png" width="800">
