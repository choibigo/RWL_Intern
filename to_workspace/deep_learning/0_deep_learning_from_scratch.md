...
그러면 출력층에서 사용하는 활성화 함수인 소프트맥스에 대해서 복습해보자.
...
- MSE와 다르게 CEE는 원래 부터 데이터 하나만을 위해 오차를 계산하기에 $1/N$이 필요 없음. 다만 실제 학습시 데이터들을 묶어서 여러번 학습하기에, 그 미니배치끼리의 평균 오차를 구할 필요는 있음.
...
각 $x_0$와 $x_1$의 값에 따른 기울기에 대한 벡터(gradient)를 구하고 보기 좋게 나타낸 것.
...
- 주로 Gradient $\nabla f$나, 발산(Divergence) $\nabla \cdot \mathbf{F}$를(벡터장이 한 점에서 퍼져 나가는지, 모여드는지를 나타냅) 표현할 때 사용함.
...
> 손실함수에서 극솟값, 최솟값, 안장점이 되는 곳에서 기울기가 0이 됨. 함수가 복잡해지면 학습 도중에 평평한 고원 같은 것들이 생길 수 있음.
...
1. 미니배치: 훈련 데이터 중 일부를 무작위로 가져옴. 이렇게 선별한 데이터를 미니배치라고 하며, 그 미니배치의 손실함수 결과값을 줄이는 것이 목표이다.
