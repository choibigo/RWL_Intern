...
- 위에 보면 RGB 3차원의 입력이라도, 한 커널 세트로 돌리면, 결국 2차원의 피쳐맵을 얻는게 끝이다.

- 2차원의 피쳐맵만으로 너무 아깝지 않은가? 너무 하나의 시각으로 정보를 압축했으니, 이를 통해 얻는게 너무 적지 않은가?

- 그래서 보통 커널 세트를 늘려서 아웃풋 피쳐맵을 많게한다.
...
- 참고로 연산하기 편하게 합성곱 데이터와 필터들을 2차원으로 펴서 계산한다.
...
- 그리고 신경망이 깊어지면, 점차 텍스쳐, 오브제 같이 더 거시적이고 고수준의 무언가를 바라보게 된다.
...
- 위 이미지로만 보면 잘 와닿지 않을 수도 있어서 더 큰 예시을 들자면:
  - 100x100 크기의 흑백 이미지가 있다고 가정하자. (패딩이 잘 돼서 이미지가 줄어들지는 않는다.)
    - 5x5 커널을 모든 픽셀을 한번씩 돌리면? 
      - 100x100x5x5 만큼 연산을 해야한다. 총 25만번 연산 필요.
    - 하지만, 3x3을 두번하면?
      - 100x100x3x3을 한번하면 9만번의 연산이다.
      - 그리고 이를 한번 더 하니 총 18만번의 연산이다.
  - 그리고 이 차이는 층이 깊어질 수록 더 커진다.
- 심지어 층이 나눠지면 ReLU를 추가해서 더 많은 것을 담게 만들 수도 있다.

- 또한 이렇게 층을 많이 쌓으면 해결해야할 복잡한 문제를 계층별로 분해하여 더 단순한 문제로 해결할 수 있다는 장점도 있다.

- 물론 깊게 만들면 학습이 어렵다는 것이 함정이지만.
...
- 모델이 깊어짐에 따라 역전파의 신호가 입력층으로 가면서 **약해질** 수 있는데, 역전파시 출력층 쪽의 싱싱한 오차를 입력층 쪽으로 산지직송으로 그대로 들어가게 함으로서 업데이트하는 것.
...
- 멀티모달 처리의 정의는 하나의 거대한 신명망이 텍스트, 이미지, 소리 등 다양한 정보를 **한번에 학습하고 처리**하는 것. 다른 모델을 조합하는 의미는 정확히 아님. 
