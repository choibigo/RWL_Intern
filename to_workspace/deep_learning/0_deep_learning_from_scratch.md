...
- 식이 약간 복잡해보이지만 여기서 끝난다. 잘 보면 3개의 항으로 위 긴 식을 나눌 수 있다.
  ...
  - 가운데 항: $2(f(x) - E[\hat{f}(x)])(E[\hat{f}(x)] - \hat{f}(x))$
    - 위 항에서 $E[\hat{f}(x)] - \hat{f}(x)$ 부분 부터 E() 밖으로 나간다고 가정하자.
      - 그러면 $E\left[E[\hat{f}(x)] - \hat{f}(x)\right]$ 이 되고. 이는 예측 평균값에 대해서 예측 값을 뺀 것들의 평균이다. 근데 잘 생각해보면 그건 그냥 `0`이다. (예를 들어 평균이 10이고, 데이터가 [8,10,12]인데 평균 - 데이터의 평균은 당연히 0이다.)
    - 그래서 **없어짐**
...
- **Variance**, 또는 $\text{Var}(\hat{f}(x))$ 예측 값에 대한 **분산**이다. 즉, 하나의 예측이 있을 때 **얼마나 많이 분포된 결과**를 내놓을 수 있냐는 것이다.
  - 신경망을 점점 깊게, 점점 **더 복잡하게** 할수록 **분산이 증가**한다. 너무 복잡한 모델을 만들면, 작은 인풋의 변화가 **큰 아웃풋의 변화**를 만들수도 있다. 이는 신경망이 지나치게 **예민**한것 (민감도가 높다고도 한다). **과적합**이 된 것이다.

- 그렇기에 **너무 깊은 모델**도 학습이 오래 걸리는 것과 별개로 **마냥 좋은 것이 아니다**.
...
- 이론상 위에 적은대로 모델이 너무 복잡, 즉 파라미터가 너무 많이 증가하면 분산이 너무 커져서 Loss가 오히려 늘어나는 과적합 현상을 겪을 것으로 예상된다.
...
- 임계점 부근에서는 훈련 데이터를 억지로 다 맞추는 '가장 뾰족하고 복잡한 함수' 하나를 간신히 찾아낸다. (Train에 대한 Loss는 적은데 Variance가 극도로 높음) (뉴런 24개)
