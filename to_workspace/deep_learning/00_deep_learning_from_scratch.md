...
- 결과: 가정이 없기 때문에 어떤 형태의 함수든 근사할 수 있는 유연성을 가지지만, 그만큼 패턴을 스스로 깨우치려면 엄청나게 많은 데이터와 학습 시간이 필요함.
...
- 결과: 데이터, 그것도 이미지 데이터에 대한 뚜렷한 가정을 미리 심어두었기 때문에 MLP에 비해 훨씬 효율적임.
...
- 가정
  - 전역적 연결: 입력된 모든 데이터 조각(Token)은 거리에 상관없이 서로 연결되어 있으며, 어디에 집중할지는 스스로 학습한다.
    - Self-Attention은 문장이나 이미지 내의 위치가 얼마나 떨어져 있든 모든 요소가 서로의 연관성을 계산함.
    - 그렇기에 멀리 떨어진 데이터 간의 문맥(Long-term dependency)을 파악하는 데 탁월, But 연산량이 엄청남.
  - 순서에 대한 무관심: 데이터가 들어오는 순서는 기본적으로 신경 쓰지 않는다. 그냥 한 바구니에 담긴 집합(Set)으로 본다.
    - Transformer는 한꺼번에 병렬로 데이터를 병렬로 받음. 그래서 RNN과 같이 순서에 대한 개념 노.
    - 덕분에 병렬 처리가 가능해져 학습 속도가 비약적으로 빨라짐. But 입력 데이터에 순서 정보를 억지로 주입해야하는 Positional Encoding 작업이 필수적임 (이건 1번째 단어, 이건 2번째 단어... 이렇게).

- Inductive Bias가 약하기에, 압도적으로 많은 데이터와 컴퓨팅 자원이 필요함. 
  - 하지만 그게 만족이 되면, 훨씬 더 정확하고 일반화된 패턴을 찾아낼 수 있음. + 범용서도 챙길 수도 있고.
