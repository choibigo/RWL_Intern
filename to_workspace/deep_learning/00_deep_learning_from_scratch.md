...
- $\frac{1}{2} \lambda W^2$ : 이 부분 을 L2 Regularization (여기서 $\|W\|_2$이 L2 norm) 이라고도 부름. 참고로 W는 전체 웨이트들의 제곱함의 제곱근은: $W = \sqrt{w_1^2+w_2^2+w_3^2+...}$ (수학적으로 $\|W\|_2$ 으로 표기하기도 함)
...
- 멀티모달 처리의 정의는 하나의 거대한 신명망이 텍스트, 이미지, 소리 등 **다양한 정보**를 **한번에 학습하고 처리**하는 것. 다른 모델을 조합하는 의미는 정확히 아님. 
...
- 가정
  ...
  - **순서에 대한 무관심**: 데이터가 들어오는 **순서는 기본적으로 신경 쓰지 않는**다. 그냥 한 바구니에 담긴 집합(Set)으로 본다.
    - Transformer는 한꺼번에 병렬로 데이터를 받음. 그래서 RNN과 같이 순서에 대한 개념 노.
