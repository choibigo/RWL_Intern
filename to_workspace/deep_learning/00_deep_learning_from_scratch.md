...
- 하지만 파라미터 수가 **훈련 데이터의 수보다 압도적**으로 많아지는 **초거대 모델 구간**으로 진입하면, 치솟았던 Loss 증가 또는 분산의 증가가 **다시 하강**하는, **두번째 하강(Second Descent)** 이 발생한다.
...
- 가정
  ...
  - **순서에 대한 무관심**: 데이터가 들어오는 **순서는 기본적으로 신경 쓰지 않는**다. 그냥 한 바구니에 담긴 집합(Set)으로 본다.
    - Transformer는 한꺼번에 병렬로 데이터를 병렬로 받음. 그래서 RNN과 같이 순서에 대한 개념 노.
