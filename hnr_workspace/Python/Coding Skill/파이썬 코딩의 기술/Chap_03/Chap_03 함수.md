...
- `*args` (가변 인자)의 등장
    - 인자 이름 앞에 `*`를 붙이면, 파이썬은 그 뒤에 오는 모든 인자를 하나의 튜플(tuple)로 묶어서 `values`에 담아줍니다.
...
- 제네레이터와 메모리 주의사항
    - 제네레이터(데이터를 하나씩 생성하는 도구)를 로 풀어서 `args`에 넘깁니다.
...
- 위치 인자(Positional Arguments)
    ...
    - 가장 단순하지만, 인자가 많아지면 "어떤 숫자가 무슨 의미인지" 알기 어렵고 순서가 바뀌면 큰일 납니다.
...
- 코드 흐름 추적
    ...
    - `sleep(0.1)`: 프로그램이 0.1초 쉽니다. 실제 시간은 `14:51:17.758683`이 되었을 것이다.
...
- 여기서 `default = {}`는 함수가 호출될 때(실행될 때) 실행됩니다. 즉, `decode`를 부를 때마다 매번 완전히 새로운 빈 딕셔너리가 메모리에 만들어진다.
